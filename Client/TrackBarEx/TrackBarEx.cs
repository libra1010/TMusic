...
namespace TrackBarEx
{
    public partial class TrackBarEx: UserControl
    {
        ...
        private bool _UseToControlVolume=false;
        [Description("是否用于音量控制")]
        public bool UseToControlVolume
        {
            get { return _UseToControlVolume; }
            set { _UseToControlVolume = value; }
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            this.Value = (int)(percent*(Maximum-Minimum));
            Invalidate();
            if (ValueChanged != null)
            {
                ValueChanged(this, e);
        ...
        {
            if ((CState == ControlState.Focused||CState==ControlState.Hover) && UseToControlVolume)
            {
                if (e.Delta > 0)
                {
                ...
                {
                    if (this.Value - SmallChange <= this.Minimum)
                    {
                        this.Value = Minimum;
                    }
                    else
